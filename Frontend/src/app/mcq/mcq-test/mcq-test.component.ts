...
@Component({
    ...
    <div class="test-page">
        ...
        <div class="start-card">
          ...
          <div class="instructions">
            ...
            <ul>
              ...
              <li>Tab switches and leaving fullscreen are actively monitored and logged.</li>
              <li>Each question has only one correct answer. The timer starts when you click Begin.</li>
            </ul>
          </div>
      ...
      <div class="test-interface" *ngIf="started && !submitted">
        ...
        <div class="test-topbar">
          <div class="test-brand">⬡ SmartHireAI — MCQ Assessment</div>
            ...
            <div class="question-card card" *ngIf="questions[currentQ]">
              ...
              <div class="q-navigation">
                ...
                <button class="btn btn-primary" *ngIf="currentQ === questions.length - 1" (click)="confirmSubmit()" [disabled]="submitting">
                   <span *ngIf="!submitting">Submit Test ✓</span>
  ...
  styles: [`
    ...
    .center-page { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 80vh; }
...
export class McqTestComponent implements OnInit, OnDestroy {
  ...
  testId!: number;
  submissionId!: number;
  loading = true; startLoading = false; submitting = false;
  ...
  loadTestData() {
    this.api.startMcqTest(this.jobId).subscribe({
      next: (test: any) => {
        this.testId = test.id;
        this.timeLimitMin = test.durationMinutes || 30;
        ...
        this.api.getRound2Questions(this.testId).subscribe({
          ...
          error: () => {
            this.startError = 'Failed to load test questions.';
            this.loading = false;
          }
        });
      },
      error: (err) => {
        this.startError = err?.error?.message || 'Not eligible for this test.';
        this.loading = false;
      }
    });
  }
  ...
  beginTest() {
    ...
    this.api.startRound2Test(this.testId).subscribe({
      next: (sid: any) => {
        this.submissionId = typeof sid === 'number' ? sid : (sid?.submissionId || this.testId);
        this.started = true;
  ...
  private setupProctoring() {
    this.visibilityHandler = () => {
      if (document.visibilityState === 'hidden' && this.started && !this.submitted) {
        this.violations++;
        this.logViolation('TAB_SWITCH', 'Switched browser tab or minimized window');
      }
    };
  ...
  private analyzeCanvasFrameForFaces(video: HTMLVideoElement): number {
    ...
    if (leftHalfSkinCount > minSideCluster && rightHalfSkinCount > minSideCluster && centerSkinCount < (leftHalfSkinCount + rightHalfSkinCount) * 0.5) {
      return 2; // Multiple people / heads present on both sides
    }
